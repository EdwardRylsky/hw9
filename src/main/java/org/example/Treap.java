 ...
 * Применение в linux kernel<br/>
 * <a href="https://www.kernel.org/doc/mirror/ols2005v2.pdf">https://www.kernel.org/doc/mirror/ols2005v2.pdf</a>
 */
public class Treap {
    public static View view = View.value;
    Node root;
    private final TreePrinter<Node> printer;
    ...
    public void addToAll(int a, int b, int value) {
        // [0 < a) [a < N)
        Node[] low = root.split(a - 1);
        if (low[1] == null) {
            low[0].addToPromiseAdd(value);
            root = low[0];
        } else {
            // [a < b] (b < N)
            Node[] high = low[1].split(b - a + 1);
    ...
    public static class Node {
        ...
        public Node(Integer value, int priority, Node left, Node right) {
        ...
        public void setSumValue(long sumValue) {
            statistic.sumValue = sumValue + (long) addPromise * size;
        }
